...
zh.server = function(fn){
    ...
    $.getJSON("../../uploads/base.json", function(data) {
        ...
        if(!zh.conf) {
            console.log("数据服务器地址为设置！");
            Base = data;
            fn();
            return;
        }
        if(!zh.conf.server || !zh.conf.server_uploads) {
            console.log("数据服务器地址为设置！");
            Base = data;
            fn();
            return;
...
//Sever 函数
var Url = {};
Url.fs  = "Zh/resources/";//fs的时候的相对地址
Url.upload = "../uploads/";
var Server = {};
//资料下载前判断
